...
static uint64_t *vmm_get_or_create_next_table(uint64_t *table, uint16_t index, uint64_t flags) {
...
int vmm_map(uint64_t *pml4, uint64_t phys, uint64_t virt, uint64_t flags) {  
    ...
    uint64_t *pdpt_table = vmm_get_or_create_next_table(pml4, pml4_index, flags);
    if (!pdpt_table) return -1;

    uint64_t *pd_table = vmm_get_or_create_next_table(pdpt_table, pdpt_index, flags);
    if (!pd_table) return -1;

    uint64_t *pt_table = vmm_get_or_create_next_table(pd_table, pd_index, flags);
...
uint64_t *vmm_init(struct limine_memmap_response *memmap_resp, struct limine_executable_address_response *executable_address_response, uint64_t hhdm_offset_val) {
    ...
    for (uint64_t i = 0; i < kernel_pages; i++) {
        if (vmm_map(pml4_virt, executable_address_response->physical_base + i * 0x1000, executable_address_response->virtual_base + i * 0x1000, flags) < 0) return NULL;
    }
